<?php
//создание таблиц в базе(скрипты)
    include_once('function.php');
    $mysqli = connect();//подключение к базе
    ...
    $ct6 ='create table users(
        ...
        mysqli_query($mysqli, $ct1);  //Выполняет запрос к базе данных
        $err = mysqli_errno($mysqli);   //Возвращает код ошибки последнего вызова функции
        if ($err){
            echo 'Error code 1:'.$err.'<br>';
            exit();
        }
        mysqli_query($mysqli, $ct2);  //Выполняет запрос к базе данных
        $err=mysqli_errno($mysqli);    //Возвращает код ошибки последнего вызова функции
        if ($err){
            echo 'Error code 2:'.$err.'<br>';
            exit();
        } 
        mysqli_query($mysqli, $ct3);  //Выполняет запрос к базе данных
        $err=mysqli_errno($mysqli);    //Возвращает код ошибки последнего вызова функции
        if ($err){
            echo 'Error code 3:'.$err.'<br>';
            exit();
        }
        mysqli_query($mysqli, $ct4);  //Выполняет запрос к базе данных
        $err=mysqli_errno($mysqli);   //Возвращает код ошибки последнего вызова функции
        if ($err){
            echo 'Error code 4:'.$err.'<br>';
            exit();
        }
        mysqli_query($mysqli, $ct5);  //Выполняет запрос к базе данных
        $err=mysqli_errno($mysqli);   //Возвращает код ошибки последнего вызова функции
        if ($err){
            echo 'Error code 5:'.$err.'<br>';
            exit();
        }
        mysqli_query($mysqli, $ct6);  //Выполняет запрос к базе данных
        $err=mysqli_errno($mysqli);   //Возвращает код ошибки последнего вызова функции
        if ($err){
