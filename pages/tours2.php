...
<?php
    $mysqli = connect();
    $selcountry = (isset ($_POST['countryid']))?$_POST['countryid'] : 0;// сохраняет значение селекта
    ...
    while ($row=mysqli_fetch_array($res, MYSQLI_NUM)){
        $selected = ($selcountry == $row[0])?'selected':' ';// сохраняет значение селекта
        echo '<option '.$selected.' value="'.$row[0].'">'.$row[1].'</option>';// сохраняет значение селекта
    }
